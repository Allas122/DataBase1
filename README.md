...
$$;
```
Предметов не может быть 20000, по этому я просто так и оставлю.
...
Как мы видим, происходит SEQ SCAN, а это значит что мы буквально перебираем всю таблицу(т.к индексов на ней нет). Что это значит ? Правильно, O(N) от количества записей.
...
2. Мы на этапе поиска даже не зашли в данные таблицы, мы просканировали только индекс и из него уже дальше поняли что доставать из таблицы.